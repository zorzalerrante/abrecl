...
define("matta_dst_map", ["d3", "matta", "topojson", "leaflet"], function (d3, matta, topojson, leaflet) {
    var mod_dst_map = function() {
        var func_dst_map = function (selection) {
            console.log('selection', selection);
            selection.each(function(json) {
                var container = null;

                if (d3.select(this).select("div.dst_map-container").empty()) {
                    
                        var div = d3.select(this).append("div")
                            .style("width", width + "px")
                            .style("height", height + "px")
                            .attr('class', 'dst_map-container');
                    ...
                    container = d3.select(this).select("div.dst_map-container");
                }
...
var draw_topojson = function() {
    ...
    reset();
        ...
        var height = 350;
            ...
            var _dataframe = null;
            func_dst_map.dataframe = function(_) {
                if (arguments.length) {
                    _dataframe = _;
            ...
            func_dst_map.property_value = function(_) {
                if (arguments.length) {
                    _property_value = _;
